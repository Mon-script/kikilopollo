          ...
          <Table.Row className="bg-white" key={index}>
            <Table.Cell>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-body-4 font-medium text-metal-600">{item.nombre_producto}</p>
                  <p className="text-body-4 font-medium text-metal-600">{item.id_podructo}</p>
                  <span className="text-body-6 font-normal text-metal-500">{item.id_salida}</span>
                </div>
              </div>
            </Table.Cell>
            <Table.Cell>
            ...
            <Table.Cell>
              <p className="text-body-5 font-medium text-metal-500">Empleado: {item.empleado}</p>
            </Table.Cell>
            <Table.Cell>
            ...
            <Table.Cell>
              <p className="text-body-5 font-medium text-metal-500">Id int empleado</p>
              <p className="text-body-6 font-normal text-metal-500">{item.empleado_id}</p>
            </Table.Cell>
            <Table.Cell>
              <Popover showDismissIcon={false} showArrow={false} className="w-52 border border-metal-100 p-2">
                ...
                <Popover.Action>
                  {user && user.role === 'admin' ? (
                    <Button type="outlineGray" size="xs" circle={true} onClick={()=>{deleteRow(item.id_salida)}}>
