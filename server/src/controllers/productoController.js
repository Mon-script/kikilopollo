...
module.exports.saveProduct = (req, res) => {
  const { id_marca, nombre_producto, img, precio_costo, precio_mayorista, precio_minorista, id_precio_especial, detalle_producto } = req.body;
  ...
  const consult = 'INSERT INTO PRODUCTO (id_marca, nombre_producto, avatar, precio_costo, precio_mayorista, precio_minorista, id_precio_especial, detalle_producto, activo) VALUES (?,?,?,?,?,?,?,?,?)';

  try {
    connection.query(consult, [id_marca, nombre_producto, img, precio_costo, precio_mayorista, precio_minorista, id_precio_especial, detalle_producto, 1], (err, result) => {
      if (err) {
        console.error(err);
        res.send(err);
      } else {
